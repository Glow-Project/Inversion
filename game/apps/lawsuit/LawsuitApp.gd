...
func _on_PatentList_item_selected(index):
	...
	$Sue.disabled = selected_patent.id != current_invention.patent_id
...
func _on_Sue_pressed():
	if selected_patent == null || current_invention == null:
		return
		
	if selected_patent.id != current_invention.patent_id:
		return
